...
void G_GhostTicker(void)
{
	...
	for(g = ghosts, p = NULL; g; g = g->next)
	{
		...
		else
		{
			if (ziptic & GZT_MOMXY)
			{
				g->oldmo.momx = (demoversion < 0x000e) ? READINT16(g->p)<<8 : READFIXED(g->p);
				g->oldmo.momy = (demoversion < 0x000e) ? READINT16(g->p)<<8 : READFIXED(g->p);
			}
			if (ziptic & GZT_MOMZ)
				g->oldmo.momz = (demoversion < 0x000e) ? READINT16(g->p)<<8 : READFIXED(g->p);
...
#define follow g->mo->tracer
		...
		{ // Even more...
			...
			if (follow)
			{
				...
				temp = (demoversion < 0x000e) ? READINT16(g->p)<<8 : READFIXED(g->p);
